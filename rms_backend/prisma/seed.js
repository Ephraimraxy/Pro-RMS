...
async function main() {
  ...
  // ── Env-controlled access codes ─────────────────────────────────────────────
  // Super Admin auth NEVER uses the DB hash — it uses SUPER_ADMIN_ACCESS_CODE
  // env var directly in serve.js. The hash stored here is intentionally null.
  // All other special departments read their initial code from env vars too.
  const GM_ACCESS_CODE  = process.env.GM_ACCESS_CODE  || 'GM-2026';
  const CEO_ACCESS_CODE = process.env.CEO_ACCESS_CODE || 'CEO-2026';
  const ICC_ACCESS_CODE = process.env.ICC_ACCESS_CODE || 'ICC-2026';
  ...
  const departments = [
    ...
    { name: 'General Manager (GM)',               type: 'Strategic',   code: 'GMR', accessCode: GM_ACCESS_CODE  },
    { name: 'CEO (Chairman)',                     type: 'Strategic',   code: 'CEO', accessCode: CEO_ACCESS_CODE },
    { name: 'Internal consult and control (ICC)', type: 'Strategic',   code: 'ICC', accessCode: ICC_ACCESS_CODE },
  ];
  ...
  // Super Admin is handled separately — its auth uses SUPER_ADMIN_ACCESS_CODE
  // env var directly in serve.js (bcrypt hash is never checked for this dept).
  // We store no hash here so there is no ambiguity about which credential is active.
  await prisma.department.upsert({
    where: { name: 'Super Admin' },
    update: {
      // Keep access code label in sync with env var on every redeploy
      accessCodeLabel: '[env: SUPER_ADMIN_ACCESS_CODE]',
      accessCodeHash: null,
    },
    create: {
      name: 'Super Admin',
  ...
  // 4. Create Initial Workflow Stages
